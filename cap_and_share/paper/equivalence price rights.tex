...
The other three hand that dividend back as rights, so that club emissions match the proposal's and temperature is held fixed, and differ in \emph{who} receives it. \emph{Variant 2} scales every allocation by one common factor, which gives the surplus to whoever already holds the most rights. That is regressive, and it can leave a member worse off than the proposal it is meant to match. Two things move at once: a member receives more rights, which helps, but the looser cap lowers the club price, and a member holding more rights than its emissions is a net seller whose revenue falls with that price. Where the second effect dominates---a country whose allocation is large relative to its own emissions---scaling everyone's rights up reduces its welfare, which is why an inequality-averse world aggregate can fall under variant 2 even as world consumption rises.

\emph{Variant 3} removes that possibility by flooring each member on the allocation option A found equivalent for it before scaling the remainder up to the proposal's emissions, so no member ends below the rights that made it indifferent. For option A the floor is its own allocation, so variant 3 coincides with variant 2 there; it bites for option B, whose joint solve moves rights away from the members option A had compensated most. \emph{Variant 4} keeps the same total but distributes the surplus over the equivalent allocation in proportion to population times marginal utility, $n_i\,c_i^{-\eta}$, with $c_i$ the country's equally-distributed-equivalent consumption under the proposal---that is, to the members where a tonne of rights buys the most welfare, which is how an inequality-averse planner would share a surplus of this kind. The four variants therefore bracket the question: variant 1 prices the dividend as climate ambition at constant welfare, and variants 2 to 4 price it as welfare at constant ambition, under a regressive, a floored and a progressive sharing rule respectively. We report each on the inequality-averse global EDE and on world mean consumption per capita.
...
% \IfFileExists{../output/equivalent_rights_option_A.tex}{\input{../output/equivalent_rights_option_A.tex}}{}
\IfFileExists{../output/equivalent_rights_combined.tex}{\input{../output/equivalent_rights_combined.tex}}{}

% Table generated by src/equivalent_rights_proposals.jl; regenerate it there,
% do not edit the numbers here.
% \IfFileExists{../output/equivalent_rights_option_B.tex}{\input{../output/equivalent_rights_option_B.tex}}{}
...
The policy-relevant result is the aggregate one. Every country can be held at exactly the welfare it obtains under a proposal while the club's carbon budget is cut by about a tenth---9.9\% under the Wolfram schedule and 9.1\% under the Duflo schedule---which takes 2100 warming down by 0.02\textdegree C and 0.04\textdegree C respectively. In variant 2, where the allocation is rescaled so that emissions match the proposal exactly, world mean consumption per capita rises under both methods---by 0.022\% (Wolfram et al.) and 0.059\% (Banerjee et al.) under option A, and 0.018\% and 0.050\% under option B. The inequality-averse global EDE moves in the same direction under option A ($+0.028\%$ and $+0.104\%$) and marginally against it under option B ($-0.006\%$ and $-0.008\%$), which says option B's allocation is slightly more regressive than option A's. All four EDE figures are an order of magnitude below the 0.026--0.054\% resolution of the joint solve, so the efficiency dividend is what the exercise identifies here; the welfare sign is not. The dividend is small in either case, and it is legible as climate ambition at constant welfare (variant 1) as readily as welfare at constant ambition (variant 2); the near-cancellation between higher abatement cost and lower damages over 2030--2100 discounted at 3\% is also why the overall stringency of the equivalent allocation is only weakly identified by the indifference conditions themselves.
