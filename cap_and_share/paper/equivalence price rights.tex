...
A third schedule, Equal Right, graduates the charge by climate vulnerability as well as by income: in 2025 it asks \$240/tCO$_2$ of the least vulnerable high-income countries and as little as \$12 of the most vulnerable low-income ones, covering 173 of our countries. Its own path escalates that charge by 16.4\% a year, which carries it past the model's backstop within a decade---by 2035 a third of its members sit at full abatement and by 2050 almost all of them do---so the price differentiation the equivalence exercise reads is concentrated in the first decade, and the scenario is far more ambitious than the other two (1.48\textdegree C in 2100, against 2.16 and 1.80). We therefore also run its 2025 levels grown at the 5\% a year assumed for the other schedules, which isolates what is distinctive about the levels themselves, and report both.
Because these proposals define $\pi_i$ differently by construction (a shared coalition schedule in one case, an income-conditional compensation mechanism in the other), the resulting country-level price factors, and hence the $\rho$ needed to match them, diverge substantially across proposals for the same country.

We look for the allocation of emission rights that, combined with a uniform world carbon price, would leave every country exactly as well off as under each proposal, where ``as well off'' means the same net present value of mean consumption per capita over 2030--2100. Writing country $i$'s allocation as
\begin{equation}
...
We solve for $\rho$ in two ways. Under the \emph{isolated} solve, each country's $\rho_i$ is found on its own, with every other country grandfathered on its proposal-scenario emissions and hence facing no net transfer; this is a partial-equilibrium device, and it is the method used for the country-by-country results of Section~\ref{sec:empirics}. Under the \emph{joint} solve, all $\rho_i$ are solved together: one allocation, one model run, every country's consumption gap read from it, and a damped secant step on each $\rho_i$ until all countries simultaneously reach the consumption the proposal gives them. The joint solve also lets the allocation set the cap and the cap set the price at every iteration, which both closes the general-equilibrium loop the isolated solve cuts and pins down the overall level of the rights. Everything reported in this section is the joint solve, whose country-level numbers are the ones that survive being applied to every member at once; Appendix~\ref{app:tables} sets the two side by side.

Each set of ratios is then read in four variants. In \emph{variant 1} the allocation is taken at face value: the club's rights add up to $\sum_{i \in S} \rho_i n_i \bar e_S$, that total is the cap, and the club price is recalibrated so that members' emissions deliver it. Every member keeps exactly the consumption the proposal gives it, by construction of $\rho$, so the dividend appears not as consumption but as a cap below the proposal's own emissions, and a cooler century at no consumption cost to anyone.

The other variants hand that dividend back as rights, so that club emissions match the proposal's and temperature is held fixed, and differ in \emph{who} receives it. \emph{Variant 2} chooses the split so as to make the worst-off member as well off as possible: it maximises the smallest relative consumption gain against the proposal, and keeps raising that floor after the last member has stopped losing. It is solved on consumption rather than on rights, because giving a member more permits does not by itself keep it whole: returning the surplus loosens the cap and lowers the club price, and a member holding more rights than its emissions is a net seller whose revenue falls with that price. The search runs over the whole allocation rather than over the surplus alone, which matters because a no-loss allocation at this total is known to exist---grandfathering every member on its own emissions under the proposal leaves each able to reproduce its autarky position and sell the remainder at the club price, which is the dominance result of Section~\ref{sec:static}---but that allocation hands some members \emph{less} than the equivalent one does, so a search that can only add to the equivalent allocation cannot reach it. We therefore start from both the equivalent and the grandfathered ratios and keep whichever ends with the higher floor. Two further sharing rules---one proportional to marginal utility, one a uniform scaling of the whole allocation---are set out with their results in Appendix~\ref{app:tables}. The variants therefore bracket the question: variant 1 prices the dividend as climate ambition at constant welfare, and the others price it as welfare at constant ambition. We report each on the inequality-averse global EDE and on world mean consumption per capita.

Table~\ref{tab:equiv_rights_targets_er5_losers} reports the result for the three schedules. For each it gives the price the proposal asks in 2030 and the two equivalent allocations: $\rho^{\mathrm{EDE}}$, which holds every member at its inequality-averse equally-distributed-equivalent consumption, and $\rho^{\mathrm{cons}}$, which holds it at its mean consumption per capita. The two differ systematically, and the difference is itself a result: for each of these three schedules matching the EDE takes fewer rights, because moving a high-priced member to the lower club price compresses its within-country distribution and the EDE counts that compression as part of the compensation, while mean consumption does not. The direction is not universal, however: under Equal Right's own 16.4\%/yr path, whose club price is far above the other schedules', the ranking reverses and the EDE-equivalent allocation is the larger one (Appendix~\ref{app:tables}). We solve on consumption throughout, for the reason given at the end of this section. Below the country rows, the two headline variants are each read under both solves, with the number of members left below the proposal on each measure.

% Table generated by src/equivalent_rights_proposals.jl; regenerate it there,
% do not edit the numbers here. The tables that separate the two solves, the
% other two variants and the Equal Right pair are in Appendix~\ref{app:tables}.
% Table 4 variants: ../output/equivalent_rights_targets_er5_losers{,_pred,_pred_formula,_pred_now}.tex (none, paper NPV, first-order formula, 2025 emissions pc)
...
One caveat on the joint solve. Holding the aggregate level at the isolated solve's value, as we do, means the population-weighted mean consumption gap is whatever that level implies and no allocation can remove it; only the spread around it is solved. That spread is brought down to 0.0017\% (Wolfram et al.) and 0.0019\% (Banerjee et al.) of each country's consumption, inside the 0.002\% target, and the residual mean gap---$+0.034\%$ and $+0.048\%$---is the aggregate surplus of club pricing at that level rather than a convergence failure. It also sets the resolution of the exercise: differences smaller than that are not identified, which the welfare differences discussed below comfortably exceed.
What the ratios sort on is the change in the carbon price each country actually faces, not its emissions per capita. Within a club the dispersion is therefore modest: because the club price sits among the proposal's own tiers rather than far below them, no country needs an extreme allocation and none is over-priced enough to accept a negative one. Under the Wolfram coalition the European Union pays \$75 under its income tier, well above the \$49.2 club price, and needs only about two fifths of an equal per capita share ($\rho = 0.38$); India, priced at \$25 and thus below the club price, needs more than a full share ($1.16$). The Banerjee et al.\ bargain, which prices every country, sorts the same way: the United States and Russia, at \$75 against a \$55.8 club price but with per capita emissions far above the world average, need $3.06$ and $1.98$, while the DRC, priced at \$10, needs $0.03$. Equal Right shows what happens when the gap runs the other way: on its own 16.4\%/yr path the club price is \$296.2/t in 2030 while the United States is asked for \$513, so the United States needs a quarter of an equal per capita share ($\rho = 0.25$) where the Banerjee et al.\ bargain gives it three; solved country by country rather than jointly, its allocation even turns \emph{negative} ($-0.24$), meaning it would pay a net fee for its permits and still prefer the club price to its own (Appendix~\ref{app:tables}). Countries outside a proposal's club are unpriced in both regimes and so have no equivalent allocation at all---the United States, Russia, Turkey, Nigeria and the DRC under the Wolfram coalition.

The policy-relevant result is the aggregate one. Every country can be held at exactly the consumption it obtains under a proposal while its club's emissions are cut---by 6.6\% under the Wolfram et al.\ schedule, 6.4\% under the Banerjee et al.\ one and 16.3\% under Equal Right at 5\%/yr---taking 2100 warming down by 0.013\textdegree C, 0.026\textdegree C and 0.040\textdegree C respectively. Handing that dividend back as rights instead raises world mean consumption per capita under every sharing rule, by 0.020\% (Wolfram et al.), 0.052\% (Banerjee et al.) and 0.182\% (Equal Right at 5\%/yr) under variant 2. What separates the rules is who gains. Variant 2, which maximises the smallest gain, leaves \emph{no} member below its proposal consumption in any of the four scenarios under either solve, and its floor---the gain of the worst-off member---is $+0.050\%$ under Wolfram et al., $+0.061\%$ under Banerjee et al.\ and $+0.197\%$ under Equal Right at 5\%/yr. The two other sharing rules, reported in Appendix~\ref{app:tables}, buy a larger aggregate gain at the cost of the floor: sharing by marginal utility produces much the largest inequality-averse gain but leaves members short, and uniform scaling is the weakest on both counts. The efficiency dividend is therefore real under every rule, while whether every member shares in it depends entirely on which rule is chosen.

Two caveats attach to the welfare measures themselves. The global EDE and world mean consumption can move in opposite directions for the same country, and the sign is governed by within-country incidence rather than by anything between countries. The carbon burden is distributed across deciles in proportion to their income shares raised to an estimated CO$_2$ income elasticity below one, which makes it regressive, and the revenue is returned in proportion to $c^{\eta}$, which with $\eta=1.5$ is regressive as well. More domestic pricing activity---a higher own price, or larger transfers---therefore widens a country's internal distribution, and less narrows it. Under the Wolfram et al.\ schedule, the high-income members move from \$75/t to a club price of \$49.2, which narrows their internal distribution and lifts their equally-distributed-equivalent consumption relative to their mean; the members priced at \$25/t move the other way. Because the solve now pins consumption, the residue shows up on the EDE instead: the joint solve's variant 1 leaves no member below its proposal consumption, yet one Wolfram et al.\ member and 33 Banerjee et al.\ members end below on the EDE---the counts reported in Table~\ref{tab:equiv_rights_targets_er5_losers}. This is why the equivalence is solved on consumption: the between-country question this paper asks is read cleanly off it, whereas the EDE carries the model's assumptions about who inside each country bears the price and receives the dividend. We still report the EDE alongside, and a member held exactly at its proposal consumption can gain or lose on the EDE through this within-country channel alone. Solving the same equivalence on the EDE instead is kept as a robustness check.
...
\section{The two solves, the remaining variants, and the Equal Right schedules}
\label{app:tables}

This appendix carries what the main table leaves out: the isolated solve beside the joint one, the two sharing rules not discussed in Section~\ref{sec:proposals}, and the Equal Right schedules on their own.

\paragraph{Isolated against joint.} The isolated allocation does not survive being applied to everyone at once. Each $\rho_i$ is solved with every other country held at its pro-rata split, so when all of them are granted together the club price moves and the partial-equilibrium answer no longer holds: under variant 1, 29 of the 47 Wolfram et al.\ members and 37 of the 179 Banerjee et al.\ members end below the consumption they obtain under the proposal itself, even though each was constructed to be exactly indifferent; the two Equal Right schedules leave 30 and 36 members short. The joint solve leaves none in any of the four scenarios. This is the clearest measure of what the partial-equilibrium shortcut costs, and the reason the main text reports the joint solve wherever country-level numbers matter.

The two agree on the aggregate but not on the distribution. Taking the equivalent allocation at face value cuts the Wolfram et al.\ club's emissions by 6.9\% (isolated) or 6.6\% (joint), and the Banerjee et al.\ club's by 6.7\% or 6.4\%; the implied 2100 temperatures differ by less than 0.01\textdegree C. Solving each country in isolation is therefore a good approximation for the aggregate. Individual ratios do move once the allocations interact through the global recycling of rights, and they move in a systematic direction: the joint solve is more generous to the large emitters and less to the small ones, raising the United States from $2.82$ to $3.06$ and Russia from $1.74$ to $1.98$ under the Banerjee et al.\ schedule, while lowering Nigeria from $0.22$ to $0.17$ and the DRC from $0.05$ to $0.03$. The reason is that the isolated solve credits each country in turn with the full efficiency gain of the market it joins, a dividend that cannot be paid to everyone at once; the joint solve, which must distribute one cap, takes it back from the countries whose consumption responds most strongly to transfers---the smallest emitters, whose allocation is large relative to their consumption.

\paragraph{Variant 3: sharing by marginal utility.} This rule distributes the surplus in proportion to population times marginal utility, $n_i\,c_i^{-\eta}$, with $c_i$ the country's equally-distributed-equivalent consumption under the proposal---that is, to the members where a tonne of rights buys the most welfare, which is how an inequality-averse planner would share a surplus of this kind. It produces much the largest inequality-averse gain of the four variants: $+0.29\%$ for Banerjee et al.\ under the isolated solve and $+0.86\%$ for Equal Right at 5\%/yr. It is not Pareto-safe, however: 38 of the 179 Banerjee et al.\ members end below their proposal consumption under the isolated solve, and 24 under the joint one.

\paragraph{Variant 4: uniform scaling.} This is the naive benchmark---one common factor applied to the whole allocation, which hands the surplus to whoever already holds the most rights. It is the weakest rule on both counts. It raises world mean consumption by less than variant 2 in every scenario, it turns the global EDE slightly negative for Wolfram et al.\ under the joint solve ($-0.002\%$), and it leaves members short: 7 of the 47 Wolfram et al.\ members, 43 of the 179 Banerjee et al.\ ones and 28 of the 173 Equal Right (5\%/yr) ones fall below their proposal consumption.

That a rule handing every member \emph{more} permits can leave some of them worse off is worth spelling out, because it is the same mechanism that forces variant 2 to search over the whole allocation rather than over the surplus alone. Returning the surplus loosens the cap, and a looser cap means a lower club price. A member whose allocation exceeds its own emissions is a net seller, and what it earns from the scheme is the value of the permits it sells, $(r_i - e_i)\,p$. Uniform scaling multiplies $r_i$ by a common factor but pushes $p$ down for everyone, and where a member's sales are large relative to its consumption the price effect dominates the quantity effect. Those members are the small, low-emitting, generously allocated ones---precisely the countries an inequality-averse measure weights most heavily. The rule is therefore regressive twice over: it directs the extra permits to the largest holders, and it devalues the permits the smallest holders were already living on. The global EDE can fall even as world mean consumption rises, which is exactly what the Wolfram et al.\ column shows.

\paragraph{The Equal Right schedules.} Table~\ref{tab:equiv_rights_er} reports the two Equal Right runs---its own 16.4\%/yr escalation and the 5\%/yr path assumed for the other proposals---on the same basis as Table~\ref{tab:equiv_rights}. Wolfram et al.\ and Banerjee et al.\ are omitted there because they appear in Table~\ref{tab:equiv_rights} already.

% Tables generated by src/equivalent_rights_proposals.jl; regenerate them there,
% do not edit the numbers here.
\IfFileExists{../output/equivalent_rights_combined.tex}{\input{../output/equivalent_rights_combined.tex}}{} % TODO specify we maximize conso, not EDE
...
\bibliography{price_rights,references}

\end{document}

% TODO Heatmap y-axis label reads "rᵢ = ρᵢ·e*ᵢ", which conflicts with the population-share definition in the text.
% TODO New references in references_jeem.bib were written from memory: Montgomery 1972, Hahn & Stavins 2011, Chichilnisky & Heal 1994, Shiell 2003, Weitzman 2014, Nordhaus 2015. Verify them before submission