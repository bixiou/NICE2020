...
% We then test the equivalence numerically using the NICE integrated assessment model \citep{youngbrun2025}, which disaggregates regional populations into income quintiles % AF: not deciles?
...
Two recent proposals offer differentiated carbon-price schedules that can be fed directly into the NICE model as country-level $\pi_i$ inputs. \citet{wolfram2025} propose a voluntary coalition of countries that would align on a tiered carbon-pricing schedule, with higher-income members facing higher price floors, paired with a shared carbon border adjustment mechanism. The report estimates that the coalition could raise close to \$200 billion per year in domestic carbon-tax revenue. \citet{duflo2025} propose a "grand bargain" in which low-and middle-income countries become eligible for climate-damage compensation, conditional on adopting a graduated domestic carbon price: \$10/tCO$_2$ for low-income countries, \$30 for lower-middle-income countries, and \$50 for upper-middle-income countries. Because the two proposals define $\pi_i$ differently by construction (a shared coalition schedule in one case, an income-conditional compensation mechanism in the other), the resulting country-level price factors, and hence the $\rho$ needed to match them, diverge substantially across proposals for the same country.
...
\begin{tabular}{lcccccc}
  ...
  \midrule
  \textbf{World rights reallocation (\%)} & \multicolumn{3}{c}{50.19\%} & \multicolumn{3}{c}{48.25\%} \\
  \textbf{World temp. 2100} & \multicolumn{3}{c}{\multirow{2}{*}{2.14 $\to$ 1.757}} & \multicolumn{3}{c}{\multirow{2}{*}{1.984 $\to$ 1.757}} \\  Autarky $\to$ uniform ($^\circ$C) & \multicolumn{3}{c}{} & \multicolumn{3}{c}{} \\
  \textbf{World welfare gain (\%)} & \multicolumn{3}{c}{0.26\%} & \multicolumn{3}{c}{3.2\%} \\
  \bottomrule
\end{tabular}
\label{tab:equivalent_rights}
\\[4pt]
{\footnotesize Note: '$<$' (resp. '$>$') indicates that welfare under uniform pricing already exceeds (resp. never reaches) the autarky benchmark over the tested raw $\rho$ range; the true equivalent ratio lies below (resp. above) the reported bound. $\Delta$ T represents the difference between the domestic autarky tax and the global uniform price in 2030.}
\end{table}

Applying the two proposals' price schedules as $\pi_i$ inputs to the NICE model yields non-losing rights allocation each country would need to be indifferent to trading its proposal-specific price for uniform pricing, reported in Table~\ref{tab:equivalent_rights}.
