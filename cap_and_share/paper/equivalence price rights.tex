...
\label{eq:rho_def}
\end{equation}
$\rho_i = 1$ means an equal per capita share of the emissions the proposal itself delivers, $\rho_i > 1$ a more generous allocation, and $\rho_i < 0$ a country that is better off under uniform pricing even while paying a net fee for the privilege.

The equivalence of Section~\ref{sec:static} holds \emph{at a given global cap}: it compares a differentiated-price regime with a uniform-price regime that delivers the same world emissions, so that only the structure of prices differs. The uniform price against which $\rho_i$ is solved is therefore not the 1.8\textdegree C path $p^*$ used in the rest of this paper, but the price $p^{\mathrm{ref}}_S$ that reproduces each proposal's own emission path. That price is far lower than $p^*$---\$18.5/tCO$_2$ in 2030 under the Wolfram schedule and \$19.8 under the Duflo schedule, against \$42.6 for $p^*$---because both proposals are much less ambitious than a 1.8\textdegree C budget. Solving against $p^*$ instead would load the difference in ambition onto $\rho$, which is exactly what the exercise holds fixed: doing so requires handing out 13\% \emph{more} rights than the proposal's own emissions merely to keep every country indifferent, and the efficiency gain disappears.
...
Each set of ratios is then read in two variants. In \emph{variant 1} the allocation is taken at face value: world rights add up to $\sum_i \rho_i n_i \bar e_S$, that total is the cap, and the uniform price is recalibrated to deliver it. Because the equivalent allocation is worth less in aggregate than the proposal's own emissions, the cap tightens and the world ends the century cooler at no welfare cost to anyone. In \emph{variant 2} the same allocation is rescaled so that world rights equal the proposal's emissions exactly; temperature is then held fixed and the whole difference shows up as welfare.
...
\\[4pt]
{\footnotesize Note: $\rho_i$ is the country's allocation as a multiple of an equal-per-capita share of the proposal's world emissions; the EU27 figure aggregates its members' rights. $\Delta p_i$ is the proposal's national price minus the uniform price in 2030 (negative: the uniform regime prices the country more heavily). $\rho_i < 0$ means the country prefers the uniform regime even while paying a net fee for its allocation. Variant 1 lets the equivalent allocation set the cap, so the uniform price is recalibrated to deliver exactly those rights; variant 2 rescales the same allocation to the proposal's own emissions, so temperature is held fixed and only allocative efficiency remains. Both proposals are priced from 2025 and capped at the backstop price. The joint solve is iterated until no country's welfare differs from its proposal value by more than 0.5\% of that value; it stops at 0.49\% for Wolfram and 0.19\% for Duflo. Its aggregate stringency is held at option A's, because the indifference conditions pin the distribution of rights but barely constrain their overall level (see text).}
\end{table}

The two methods agree on the aggregate. The equivalent allocation is worth 34.8\% (option A) or 33.4\% (option B) less than the emissions the Wolfram schedule itself delivers, and 21.8\% or 20.5\% less for the Duflo schedule; the implied 2100 temperatures differ between methods by 0.01\textdegree C. Solving each country in isolation is therefore a good approximation for world totals, even though individual ratios move appreciably once the allocations are allowed to interact through the global recycling of rights: the joint solve raises China from $-0.10$ to $0.51$ and the EU27 from $-0.03$ to $0.36$, and lowers the United States from $6.68$ to $5.40$.

What the ratios sort on is the change in the carbon price each country actually faces, not its emissions per capita. Under the Wolfram coalition the United States and Russia are non-members and price nothing at all; moving them to a uniform \$18.5/tCO$_2$ in 2030 is costly for them, and they require four to seven times an equal per capita share before they will accept it. The EU27, China, Brazil and Indonesia pay \$50--75 under their income tier, well above the equivalent uniform price, so they need little or nothing---and Nigeria, the DRC and Brazil come out with \emph{negative} ratios, meaning they would hand over a net fee and still prefer the uniform regime, because cheaper abatement and lower damages outweigh what they give up in transfers. The static per capita formula, which predicts $0.23$--$0.80$ for all of these countries alike, cannot see any of this.

The policy-relevant result is the aggregate one. Every country can be held at exactly the welfare it obtains under a proposal while the world's carbon budget is cut by a fifth to a third, which takes 2100 warming down by 0.24\textdegree C under the Wolfram schedule and 0.12\textdegree C under the Duflo schedule. In variant 2, where the allocation is rescaled so that emissions match the proposal exactly, the world welfare difference is essentially nil ($-0.01\%$ under option B). That is not a failure of the argument but its arithmetic counterpart: over 2030--2100 discounted at 3\%, the higher abatement cost and the lower damages of a tighter cap very nearly offset in the aggregate, so the efficiency dividend of uniform pricing is legible as climate ambition at constant welfare rather than as welfare at constant ambition. The same near-cancellation means the overall stringency of the equivalent allocation is only weakly identified by the indifference conditions themselves, which is why option B holds the aggregate level at option A's value and solves the distribution.
